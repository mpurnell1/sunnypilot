...
def setup_sm_mock(mocker, nav_limit_kph=0, nav_valid=True):
...
class TestSpeedLimitResolverValidation(OpenpilotTestCase):
  ...
  @parameterized.expand(list(Policy), names=["policy"])
  def test_nav_fallback_when_policy_sources_empty(self, resolver_class, policy, mocker):
    resolver = resolver_class()
    resolver.policy = policy
  ...
  @parametrized_policies
  def test_nav_never_preempts_policy_sources(self, resolver_class, policy, sm_key, function_key, mocker):
    resolver = resolver_class()
    resolver.policy = policy
    sm_mock = setup_sm_mock(mocker, nav_limit_kph=50)
    source_speed_limit = sm_mock[sm_key].speedLimit
  ...
  def test_invalid_nav_ignored(self, resolver_class, mocker):
    resolver = resolver_class()
